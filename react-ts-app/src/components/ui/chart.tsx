      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item: Record<string, unknown>, index: number) => {
            ...
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            const indicatorColor = color || (item as Record<string, unknown>).payload?.fill || (item as Record<string, unknown>).color;

            return (
              <div
                key={(item as Record<string, unknown>).dataKey as string}
                className={cn(
              ...
              >
                {formatter && (item as Record<string, unknown>)?.value !== undefined && (item as Record<string, unknown>).name ? (
                  formatter((item as Record<string, unknown>).value, (item as Record<string, unknown>).name, item, index, (item as Record<string, unknown>).payload)
                ) : (
                  <>
                    {itemConfig?.icon ? (
                      <itemConfig.icon />
                    ...
                    >
                      <div className="grid gap-1.5">
                        {nestLabel ? tooltipLabel : null}
                        <span className="text-muted-foreground">{itemConfig?.label || (item as Record<string, unknown>).name}</span>
                      </div>
          ...
          >
            {itemConfig?.icon && !hideIcon ? (
              ...
              <div
                ...
                style={{
                  backgroundColor: (item as Record<string, unknown>).color as string,
                }}
              />
            )}
            {itemConfig?.label}
